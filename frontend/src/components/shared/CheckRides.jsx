            ...
            loader ? (<Loader/>) : (
                ...
                <div className='flex flex-col justify-between items-center mt-16 sm:flex-row'>
                    <div className='ml-2 mt-8 flex flex-col'>
                        <h1 className='text-2xl min-w-[40vw] font-semibold mb-6 lg:text-3xl'>Available Rides:</h1>
                        <div className='flex flex-col gap-4'>
                            {
                                rides.map((ride) => (
                                    <div key={ride._id} className='bg-[rgb(64,64,64)] text-white p-4 rounded-md'>
                                        <div className='flex justify-between text-[1.4rem] '>
                                            <div>PickUp: <span className='font-bold'>{ride.pickup}</span> </div>
                                            <div>Destination: <span className='font-bold'>{ride.destination}</span> </div>
                                        </div>
                                        <div className='flex justify-between'>
                                            <div>Price: <span className='font-bold'>${ride.fare}</span> </div>
                                            <div>Duration: <span className='font-bold'>{ride.duration}</span> </div>
                                        </div>
                                        <div className='w-full mt-4 flex justify-end'>
                                            <button className='mr-0 border-[#4d878f] border bg-none px-4 py-2 rounded-md hover:bg-[#4d878f] hover:duration-500' >Book</button>
                                        </div>
                                    </div>
                                ))
                            }
